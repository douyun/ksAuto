...
class OrderNoSearcher(object):
    ...
    def _init_client_info_dict(self):
        ...
        for row in csv.DictReader(f):
            key = (row[u'付款客户'.encode('gbk')], row[u'服务地点'.encode('gbk')], row[u'零件号'.encode('gbk')],
                   row[u'零件名称'.encode('gbk')])
...
class CommCollectorForSundan(object):
    ...
    def _process_task_date(self, line):
        ...
        for date in dates:
            ...
            if m:
                taskDates.append(
                    m.group(1).strip().replace(u'：', u':').replace(u'~', u'-').replace(u'.', u':').replace(u' ',
                                                                                                           '').replace(
                        u'－', ':').replace(u'—', ':').replace(u'_', ':'))
        if len(taskDates) > 1:
    ...
    def collect_all_rows(self):
        ...
        for row in rows[1:]:
            ...
            for line in rowStr.split('\n'):
                eachItemNames = ['vendorName', 'testDate', 'partName', 'partNo', 'batchNo', 'vendorNo', 'testSum',
                                 'okSum', 'badSum', 'badType', 'serviceDetail', 'serviceAddr', 'dayNight']
                ...
                if re.search(PATTERN.get('testPerson'), line):
                    flag = True
                    continue
                if flag and match_str(PATTERN.get('taskDate'), line) == '' and match_str(PATTERN.get('testHour'),
                                                                                         line) == '':
            ...
            for testItem in testItems:
                if len(testItem) == len(eachItemNames):
                    if taskHourflag:
                        testItem.extend([self._rowDict['testPerson'], self._rowDict['taskDate'],
                                         self._get_workhour(self._rowDict['testHour']), self._rowDict['remark'],
    ...
    def save_collect_result(self, testItems):
        ...
        writer = csv.writer(f)
        testItems = [[u'订单号', u'供应商名称', u'录入方式', u'序号', u'检验日期', u'零件名称', u'零件号', u'批次号', u'供应商批次', u'检验总数', u'合格数量',
                      u'不良数量', u'不良类型', u'服务明细', u'服务地点', u'班次',
